...
def create_model(supervised = False,finetuning = False, old_model = None):
    if (old_model is None and supervised == True):
        ...
        model.load_weights("models/sim_default/controlexps/model_SL_very_deep_bn_aug_full")
        return model

    elif (supervised == False and not old_model is None):
        model = Sequential()
        model.add(Conv2D(64, 3, 3, border_mode='same', init='uniform', name = 'conv1', input_shape=(board_size, board_size, channel_size), trainable = not finetuning))
        ...
        model.add(Conv2D(128, 3, 3, border_mode='same', init='uniform', name = 'conv2', trainable = not finetuning))
        ...
        model.add(Conv2D(256, 3, 3, border_mode='same', init='uniform', name = 'conv3', trainable = not finetuning))
        ...
        model.add(BatchNormalization(name = 'bn5', trainable = not finetuning))
        model.add(Activation('relu', name = 'rl5', trainable = not finetuning))
        model.add(Dense(16, init='uniform', name = 'fc3', trainable = not finetuning))
        model.add(BatchNormalization(name = 'bn6', trainable = not finetuning))
        model.add(Activation('relu', name = 'rl6', trainable = not finetuning))
        ...
        model.get_layer('conv3').set_weights(old_model.get_layer('conv3').get_weights())
        ...
        model.get_layer('bn5').set_weights(old_model.get_layer('bn5').get_weights())
        model.get_layer('rl5').set_weights(old_model.get_layer('rl5').get_weights())
        model.get_layer('fc3').set_weights(old_model.get_layer('fc3').get_weights())
        model.get_layer('bn6').set_weights(old_model.get_layer('bn6').get_weights())
        model.get_layer('rl6').set_weights(old_model.get_layer('rl6').get_weights())
    ...
    def run_through_replay(self):
        if (self.replays.filled or self.replays.iter >= replay_start_train):
            ...
            for replay_iter in xrange(self.replays.sample_size):
                ...
                y[replay_iter][a[replay_iter]] = r[replay_iter]
                if not (sim_state == 2048).any():
                    y[replay_iter][a[replay_iter]] += discount * self.Q_max_from_target(sim_state)

...
if __name__ == '__main__':
    ...
    SL_states, SL_labels = cp.load(open('SL_data','rb'))
    ...
    history = supervised_model.fit(SL_states, np.eye(4)[SL_labels.astype('int')], 64, nb_epoch=128, verbose=2, validation_data=(SL_val_states, np.eye(4)[SL_val_labels.astype('int')]), shuffle=True, callbacks=[keras.callbacks.EarlyStopping(min_delta=0.0005, patience=5), keras.callbacks.ProgbarLogger(), keras.callbacks.ModelCheckpoint('models/sim_default/controlexps/model_SL_very_deep_bn_aug_full', 'val_loss', save_best_only=True)])

    # # Save SL model
    # filename = 'models/sim_' + str(folder_num) + '/controlexps/model_SL_deep_aug'

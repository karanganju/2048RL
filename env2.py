...
class Agent(object):
	...
	def __init__(self, env, capped = False, extra_feats = False):
		self.env = env
	...
	def take_step(self, step):
		...
		if (self.capped):
			return self.game_status, self.capped_rewards(prev_score)
		else:
	...
	def capped_rewards(self, prev_score):
		reward = self.env.score - prev_score
		if (self.game_status == 1):
			reward += 1000
		elif (self.game_status == -1):
			reward -= 1000
		if (reward > 0):
			return math.floor(math.log(reward,2))
		elif (reward == 0):
			return 0
		else :
			return -1

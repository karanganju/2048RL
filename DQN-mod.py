...
class Replays(object):
    ...
    def __init__(self, size, ssize):
        ...
        self.iter = -1
        self.filled = False
        self.s = np.zeros([self.cap, input_size], dtype=int)
        self.a = np.zeros([self.cap, 1], dtype=int)
        self.r = np.zeros([self.cap, 1], dtype=float)
        self.s_new = np.zeros([self.cap, input_size], dtype=int)

    def sample(self):
        ret_s = np.zeros([self.sample_size, input_size], dtype=int)
        ret_a = np.zeros([self.sample_size], dtype=int)
        ret_r = np.zeros([self.sample_size])
        ret_s_new = np.zeros([self.sample_size, input_size], dtype=int)
...
class DQN(object):
    ...
    def run_through_replay(self):
        if (self.replays.filled or self.replays.iter >= replay_start_train):
            ...
            for replay_iter in xrange(self.replays.sample_size):
                sim_state = s_new[replay_iter].reshape([1,input_size])
                y[replay_iter] = self.model.predict(s[replay_iter].reshape([1,input_size]))
    ...
    def evaluate(self):
        ...
        for validation_iter in xrange(self.validation_set.cap):
            sim_state = s_new[validation_iter].reshape([1,input_size])
            y[validation_iter] = self.model.predict(s[validation_iter].reshape([1,input_size]))
